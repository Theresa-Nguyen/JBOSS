...
$EAP_HOME/bin/standalone.sh -b $ipAddress -bprivate $IP_ADDR -bmanagement $IP_ADDR --server-config=standalone-ha.xml -Djboss.jgroups.azure_ping.storage_account_name=$STORAGE_ACCOUNT_NAME -Djboss.jgroups.azure_ping.storage_access_key=$STORAGE_ACCESS_KEY -Djboss.jgroups.azure_ping.container=jgroupcontainer > /var/log/eap7.log 2>&1 &
